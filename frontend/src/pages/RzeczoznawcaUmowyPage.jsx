...
import {
  RZECZ_UMOWY_BASE_PATH,
  RZECZ_UMOWY_TABS,
  isValidUmowyTabSlug,
  umowyTabSlugToStatus,
} from '../lib/rzeczoznawcaUmowyTabs'
...
function RzeczoznawcaStatusPill({ status }) {
  return (
    <span className={rzeczoznawcaStatusPillClass(status)} title="Status rzeczoznawcy">
      {status}
    </span>
  )
}
...
export function RzeczoznawcaUmowyPage() {
  ...
  const umowySubTab = useMemo(() => {
    const s = umowyTabSlugToStatus(tabSlug)
    if (s) return s
    return 'dostarczono'
  }, [tabSlug])

  const currentTabLabel = useMemo(
    () => RZECZ_UMOWY_TABS.find((t) => t.status === umowySubTab)?.label ?? umowySubTab,
    [umowySubTab],
  )

  useEffect(() => {
    if (tabSlug && !isValidUmowyTabSlug(tabSlug)) {
      navigate(`${RZECZ_UMOWY_BASE_PATH}/dostarczone`, { replace: true })
    }
  }, [tabSlug, navigate])
  ...
  const statusCounts = useMemo(() => {
    const c = { dostarczono: 0, 'w trakcie weryfikacji': 0, 'przekazano do kancelarii': 0 }
    for (const row of rows) {
      const s = getRzeczoznawcaStatusValue(row)
      if (s in c) c[s] += 1
    }
    return c
  }, [rows])
  ...
  const statusFilteredRows = useMemo(() => {
    return rows.filter((row) => getRzeczoznawcaStatusValue(row) === umowySubTab)
  }, [rows, umowySubTab])
  ...
  }, [statusFilteredRows, codeNeedle, agentNeedle])
  ...
  const showFilterEmpty =
    ...
    (codeNeedle.length > 0 || agentNeedle.length > 0)
  const showSubTabEmpty = !loading && !err && rows.length > 0 && statusFilteredRows.length === 0
  const showTableEmpty = !loading && !err && rows.length === 0

  return (
    <RzeczoznawcaLayout title="Umowy">
      <section className="dash-panel" style={{ maxWidth: '100%' }}>
        <div className="dash-panel__head">
          <h2 className="dash-panel__title dash-panel__title--kody">Umowy</h2>
        ...
        {!err ? (
          <div
            ...
            role="tablist"
            aria-label="Kategorie umów według statusu rzeczoznawcy"
            style={{ marginBottom: '1rem' }}
          >
            {RZECZ_UMOWY_TABS.map(({ slug, status, label }) => {
              ...
              return (
                <NavLink
                  ...
                  role="tab"
                  aria-selected={umowySubTab === status}
                  className={({ isActive }) =>
                    isActive ? 'dash__tab dash__tab--active' : 'dash__tab'
                  }
                >
                  {label}
                  <span className="dash-umowy-subtabs__count" aria-hidden>
                    {n}
                  </span>
                </NavLink>
              )
        ...
        {!err ? (
          <div className="dash-code-toolbar" style={{ flexWrap: 'wrap', gap: '0.75rem' }}>
            <div className="dash-code-filter" style={{ minWidth: '12rem', flex: '1 1 14rem' }}>
              ...
              <label htmlFor="rzeczoznawca-umowy-code-filter" className="visually-hidden">
                Filtruj po kodzie kalkulatora
              </label>
              <input
                id="rzeczoznawca-umowy-code-filter"
                type="search"
            ...
            <div className="dash-code-filter" style={{ minWidth: '12rem', flex: '1 1 14rem' }}>
              ...
              <label htmlFor="rzeczoznawca-umowy-agent-filter" className="visually-hidden">
                Filtruj po handlowcu
              </label>
              <input
                id="rzeczoznawca-umowy-agent-filter"
                type="search"
                className="dash-code-filter__input"
            ...
            {codeNeedle || agentNeedle ? (
              <p className="dash-code-filter__hint">
                Wyniki: {filteredRows.length} z {statusFilteredRows.length} (w zakładce)
              </p>
            ) : null}
          </div>
        ) : null}
        {loading ? (
          ...
          <div className="dash-table-wrap">
            <table className="dash-table">
              ...
              <tbody>
                {showFilterEmpty || showSubTabEmpty || showTableEmpty ? (
                  <tr>
                    <td colSpan={7} className="dash-muted" style={{ textAlign: 'center', padding: '1.5rem' }}>
                      {showFilterEmpty ? (
                        <>Brak umów pasujących do filtrów w tej podzakładce.</>
                      ) : showSubTabEmpty ? (
                        <>
                          Brak umów ze statusem „{currentTabLabel}”. Zmień status w podglądzie
                          innej umowy lub wybierz inną podzakładkę.
                        </>
                      ) : (
                        <>
                          Brak umów z gotowym BO albo brak uprawnień. Gdy back office dokończy weryfikację, umowa pojawi się
                          tutaj.
                        </>
                      )}
                    </td>
                ...
                {filteredRows.map((row) => {
                  ...
                  const agent = getAgentDisplay(row)
                  const st = getRzeczoznawcaStatusValue(row)
                  return (
                    <tr key={row.id}>
                      ...
                      <td>
                        <RzeczoznawcaStatusPill status={st} />
                      </td>
                      <td>
