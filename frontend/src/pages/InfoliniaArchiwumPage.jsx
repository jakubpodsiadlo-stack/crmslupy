...
export function InfoliniaArchiwumPage() {
  ...
  async function restoreRow(id) {
    if (!window.confirm('Przywrócić ten rekord na listę kodów?')) return
    setRestoringId(id)
    setErr(null)
    const { error } = await supabase.from('first_lead').update({ archived_at: null }).eq('id', id)
    setRestoringId(null)
    if (error) {
  ...
  return (
    <DashboardLayout title="Archiwum">
        ...
        {!err ? (
          <div className="dash-code-toolbar">
            <div className="dash-code-filter">
              ...
              <label htmlFor="archive-code-filter" className="visually-hidden">
                Filtruj po kodzie
              </label>
              <input
                id="archive-code-filter"
                type="search"
        ...
        {loading ? (
          ...
          <div className="dash-table-wrap">
            <table className="dash-table">
              <thead>
                <tr>
                  <th scope="col">Data archiwizacji</th>
                  <th scope="col">Data kodu</th>
                  <th scope="col">Kod</th>
              ...
              <tbody>
                  ...
                  <tr>
                    <td colSpan={6} className="dash-muted" style={{ textAlign: 'center', padding: '1.5rem' }}>
                      {showFilterEmpty ? (
                        ...
                        <>
                          Archiwum jest puste. Zweryfikowane wpisy z listy KODY trafiają tu automatycznie.
                        </>
                      )}
                    </td>
                  </tr>
                ...
                {filteredRows.map((row) => {
                  ...
                  return (
                    <tr key={row.id}>
                      <td>{formatDt(row.archived_at)}</td>
